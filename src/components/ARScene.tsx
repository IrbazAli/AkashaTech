...
export default function ARScene({ onExit }: ARSceneProps) {
  ...
  const targetFloorYRef = useRef<number>(13.0); // 13.0 = Ground floor
  ...
  useEffect(() => {
    ...
    function playerCollisions() {
      if (!spaceshipGroup) return false;
      let collisionOccurred = false;
      playerOnFloor = false;

      // 1. Floor Raycasting (Gravity & Grounding)
      floorRaycaster.set(
        new THREE.Vector3(playerCollider.start.x, playerCollider.start.y + 0.5, playerCollider.start.z),
        new THREE.Vector3(0, -1, 0)
      );

      const floorHits = floorRaycaster.intersectObject(spaceshipGroup, true);
      const validFloorHit = floorHits.find(hit => {
        const name = hit.object.name.toLowerCase();
        const isIgnored = name.includes('door') || name.includes('gate') || name.includes('glass');
        return !isIgnored;
      });

      if (validFloorHit && validFloorHit.distance < 1.0) {
        playerOnFloor = true;
      ...
      if (playerVelocity.x !== 0 || playerVelocity.z !== 0) {
        ...
        wallRaycaster.set(
          new THREE.Vector3(playerCollider.start.x, playerCollider.start.y + 1.0, playerCollider.start.z),
          moveDir
        );
        const wallHits = wallRaycaster.intersectObject(spaceshipGroup, true);
        ...
        if (validWallHit && validWallHit.distance < 0.5) {
          playerVelocity.x = 0;
          playerVelocity.z = 0;
          collisionOccurred = true;
          const pushBack = moveDir.clone().multiplyScalar(validWallHit.distance - 0.6);
          playerCollider.translate(pushBack);
        }
      }
    ...
    renderer.setAnimationLoop(() => {
      ...
      if (controls.isLocked) {
        ...
        for (let i = 0; i < STEPS_PER_FRAME; i++) {
          ...
          if (!playerOnFloor) {
            playerVelocity.y -= 30 * dt; // GRAVITY
    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      {inTube && (
        <div style={{
          ...
          <button
            onClick={() => targetFloorYRef.current = 13.0} // Floor 1 (Ground)
            style={{
              padding: '12px 24px',
              backgroundColor: targetFloorYRef.current === 13.0 ? '#00ffff' : '#003366',
              color: targetFloorYRef.current === 13.0 ? '#000' : '#fff',
              border: '1px solid #00ffff',
              borderRadius: '8px',
              cursor: 'pointer',
              fontWeight: 'bold',
