...
export default function PrivateRoomScene() {
  ...
  useEffect(() => {
    ...
    const ambientLight = new THREE.AmbientLight(0x111116, 1.0); // Dim cool ambient
    ...
    loader.load('/models/updated cave.glb', (gltf) => {
      ...
      cave.traverse((child) => {
        if (child.name === 'Leg part' || child.name === 'Seat part') {
          child.scale.set(2.0, 2.0, 2.0); // Make it large enough
        }
      });
      // Octree disabled per request
      cave.updateMatrixWorld(true);

      // Spawn player high enough so they fall perfectly onto the floor instead of clipping through it
      playerCollider.current.start.set(-2, 2.2, -1);
      playerCollider.current.end.set(-2, 2.8, -1);
    ...
    const animate = () => {
      ...
      if (avatarGroup.current && convStateRef.current === 'WALKING_TO_SOFA') {
        ...
        if (dist > 0.1) {
          ...
          // Reached sofa
          setConvState('STORY_SOFA');
          avatarGroup.current.rotation.y = 0; // Face outward from the sofa
          
          // Force perfect sitting position (lower height, forward Z)
          // Adjusted upward and slightly forward
          avatarGroup.current.position.set(-0.5, 0.05, -2.75);
          
          // Lock player view and height!
          playerCollider.current.start.set(-0.5, 0.2, -1.5); 
          playerCollider.current.end.set(-0.5, 0.7, -1.5);
      ...
      if (controls.isLocked) {
        const speedDelta = deltaTime * 15; // flat speed without gravity
      ...
      // Lock player Y to a flat plane since octree physics is removed
      playerCollider.current.start.y = 2.2;
      playerCollider.current.end.y = 2.8;

      // Failsafe: if player falls through the floor, respawn them
      if (playerCollider.current.start.y < -5) {
         playerCollider.current.start.set(-2, 2.2, -1);
         playerCollider.current.end.set(-2, 2.8, -1);
         playerVelocity.current.set(0, 0, 0);
      }

      // Raycast floor snapping for AI Character - disabled to remove octree physics
      if (avatarGroup.current && convStateRef.current !== 'STORY_SOFA') {
         // avatarGroup.current.position.y = 0;
      }
  ...
  const loadCharacter = (type: 'wife' | 'dad') => {
    ...
    Promise.all([
      loadGltf('/models/animations.glb'),
      loadFbx('/models/Walking.fbx').catch(() => ({ animations: [] })),
      loadFbx('/models/Sitting Talking.fbx').catch(() => ({ animations: [] })),
      loadGltf(charFile)
